...
<a href="exec_linux.go#" id="menu-button"><span id="menu-button-arrow">&#9661;</span></a>
...
<span id="L62" class="ln">    62&nbsp;&nbsp;</span>type SysProcAttr struct {
<span id="L63" class="ln">    63&nbsp;&nbsp;</span>	Chroot     string      <span class="comment">// Chroot.</span>
<span id="L64" class="ln">    64&nbsp;&nbsp;</span>	Credential *Credential <span class="comment">// Credential.</span>
<span id="L65" class="ln">    65&nbsp;&nbsp;</span>	<span class="comment">// Ptrace tells the child to call ptrace(PTRACE_TRACEME).</span>
<span id="L66" class="ln">    66&nbsp;&nbsp;</span>	<span class="comment">// Call runtime.LockOSThread before starting a process with this set,</span>
<span id="L67" class="ln">    67&nbsp;&nbsp;</span>	<span class="comment">// and don&#39;t call UnlockOSThread until done with PtraceSyscall calls.</span>
<span id="L68" class="ln">    68&nbsp;&nbsp;</span>	Ptrace bool
<span id="L69" class="ln">    69&nbsp;&nbsp;</span>	Setsid bool <span class="comment">// Create session.</span>
<span id="L70" class="ln">    70&nbsp;&nbsp;</span>	<span class="comment">// Setpgid sets the process group ID of the child to Pgid,</span>
<span id="L71" class="ln">    71&nbsp;&nbsp;</span>	<span class="comment">// or, if Pgid == 0, to the new child&#39;s process ID.</span>
<span id="L72" class="ln">    72&nbsp;&nbsp;</span>	Setpgid bool
<span id="L73" class="ln">    73&nbsp;&nbsp;</span>	<span class="comment">// Setctty sets the controlling terminal of the child to</span>
<span id="L74" class="ln">    74&nbsp;&nbsp;</span>	<span class="comment">// file descriptor Ctty. Ctty must be a descriptor number</span>
<span id="L75" class="ln">    75&nbsp;&nbsp;</span>	<span class="comment">// in the child process: an index into ProcAttr.Files.</span>
<span id="L76" class="ln">    76&nbsp;&nbsp;</span>	<span class="comment">// This is only meaningful if Setsid is true.</span>
<span id="L77" class="ln">    77&nbsp;&nbsp;</span>	Setctty bool
<span id="L78" class="ln">    78&nbsp;&nbsp;</span>	Noctty  bool <span class="comment">// Detach fd 0 from controlling terminal.</span>
<span id="L79" class="ln">    79&nbsp;&nbsp;</span>	Ctty    int  <span class="comment">// Controlling TTY fd.</span>
<span id="L80" class="ln">    80&nbsp;&nbsp;</span>	<span class="comment">// Foreground places the child process group in the foreground.</span>
<span id="L81" class="ln">    81&nbsp;&nbsp;</span>	<span class="comment">// This implies Setpgid. The Ctty field must be set to</span>
<span id="L82" class="ln">    82&nbsp;&nbsp;</span>	<span class="comment">// the descriptor of the controlling TTY.</span>
<span id="L83" class="ln">    83&nbsp;&nbsp;</span>	<span class="comment">// Unlike Setctty, in this case Ctty must be a descriptor</span>
<span id="L84" class="ln">    84&nbsp;&nbsp;</span>	<span class="comment">// number in the parent process.</span>
<span id="L85" class="ln">    85&nbsp;&nbsp;</span>	Foreground bool
<span id="L86" class="ln">    86&nbsp;&nbsp;</span>	Pgid       int <span class="comment">// Child&#39;s process group ID if Setpgid.</span>
<span id="L87" class="ln">    87&nbsp;&nbsp;</span>	<span class="comment">// Pdeathsig, if non-zero, is a signal that the kernel will send to</span>
<span id="L88" class="ln">    88&nbsp;&nbsp;</span>	<span class="comment">// the child process when the creating thread dies. Note that the signal</span>
<span id="L89" class="ln">    89&nbsp;&nbsp;</span>	<span class="comment">// is sent on thread termination, which may happen before process termination.</span>
<span id="L90" class="ln">    90&nbsp;&nbsp;</span>	<span class="comment">// There are more details at https://go.dev/issue/27505.</span>
<span id="L91" class="ln">    91&nbsp;&nbsp;</span>	Pdeathsig    Signal
<span id="L92" class="ln">    92&nbsp;&nbsp;</span>	Cloneflags   uintptr        <span class="comment">// Flags for clone calls.</span>
<span id="L93" class="ln">    93&nbsp;&nbsp;</span>	Unshareflags uintptr        <span class="comment">// Flags for unshare calls.</span>
<span id="L94" class="ln">    94&nbsp;&nbsp;</span>	UidMappings  []SysProcIDMap <span class="comment">// User ID mappings for user namespaces.</span>
<span id="L95" class="ln">    95&nbsp;&nbsp;</span>	GidMappings  []SysProcIDMap <span class="comment">// Group ID mappings for user namespaces.</span>
<span id="L96" class="ln">    96&nbsp;&nbsp;</span>	<span class="comment">// GidMappingsEnableSetgroups enabling setgroups syscall.</span>
<span id="L97" class="ln">    97&nbsp;&nbsp;</span>	<span class="comment">// If false, then setgroups syscall will be disabled for the child process.</span>
<span id="L98" class="ln">    98&nbsp;&nbsp;</span>	<span class="comment">// This parameter is no-op if GidMappings == nil. Otherwise for unprivileged</span>
<span id="L99" class="ln">    99&nbsp;&nbsp;</span>	<span class="comment">// users this should be set to false for mappings work.</span>
<span id="L100" class="ln">   100&nbsp;&nbsp;</span>	GidMappingsEnableSetgroups bool
<span id="L101" class="ln">   101&nbsp;&nbsp;</span>	AmbientCaps                []uintptr <span class="comment">// Ambient capabilities.</span>
<span id="L102" class="ln">   102&nbsp;&nbsp;</span>	UseCgroupFD                bool      <span class="comment">// Whether to make use of the CgroupFD field.</span>
<span id="L103" class="ln">   103&nbsp;&nbsp;</span>	CgroupFD                   int       <span class="comment">// File descriptor of a cgroup to put the new process into.</span>
<span id="L104" class="ln">   104&nbsp;&nbsp;</span>	<span class="comment">// PidFD, if not nil, is used to store the pidfd of a child, if the</span>
<span id="L105" class="ln">   105&nbsp;&nbsp;</span>	<span class="comment">// functionality is supported by the kernel, or -1. Note *PidFD is</span>
<span id="L106" class="ln">   106&nbsp;&nbsp;</span>	<span class="comment">// changed only if the process starts successfully.</span>
